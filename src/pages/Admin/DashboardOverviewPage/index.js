...
const DashboardOverviewPage = () => {
    ...
    const columns = [
        ...
        { field: 'status', headerName: 'Status', width: 180, renderCell: (params) => ( <StatusSelector value={params.value} onChange={(e) => handleUpdateOrderStatus(params.row.id, e.target.value)} onClick={(e) => e.stopPropagation()} > <option value="Pendente">Pendente</option> <option value="Em Preparo">Em Preparo</option> <option value="Saiu para Entrega">Saiu para Entrega</option> <option value="Concluído">Concluído</option> <option value="Cancelado">Cancelado</option> </StatusSelector> ) },
        { field: 'actions', headerName: 'Ações', width: 150, sortable: false, renderCell: (params) => ( <button onClick={(e) => { e.stopPropagation(); handlePrintOrder(params.row.id); }} style={{ cursor: 'pointer', background: 'none', border: 'none', color: '#7c3aed', padding: '10px' }} aria-label="Imprimir Recibo" > <FaPrint size={20} /> </button> ), },
    ...
    if (isAuthLoading) {
        return <PageWrapper><LoadingText>A verificar os dados da sua loja...</LoadingText></PageWrapper>;
    }
    
    if (!tenant) {
        return <PageWrapper><LoadingText>Bem-vindo! As informações da sua nova loja estão a ser preparadas. A página irá carregar em breve.</LoadingText></PageWrapper>;
    }
    ...
    return (
        <PageWrapper>
            ...
            <Box sx={{ width: '100%', bgcolor: 'background.paper', borderRadius: '8px', boxShadow: 1, overflow: 'hidden' }}>
                <Tabs value={selectedStatusTab} onChange={handleTabChange} variant={isMobile ? 'scrollable' : 'standard'} scrollButtons="auto" allowScrollButtonsMobile centered={!isMobile} indicatorColor="primary" textColor="primary" >
                ...
                <DesktopDataGrid>
                    <Box sx={{ height: 600, width: '100%', overflow: 'hidden' }}>
                        <DataGrid rows={filteredOrders} columns={columns} initialState={{ columns: { columnVisibilityModel: { id: false } } }} pageSizeOptions={[5, 10, 20]} loading={loadingOrders} localeText={{ noRowsLabel: 'Nenhum pedido para exibir nesta categoria' }} />
                    </Box>
                </DesktopDataGrid>
                <MobileCardList>
